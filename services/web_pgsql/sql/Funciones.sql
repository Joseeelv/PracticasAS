...
CREATE OR REPLACE FUNCTION Insertar_Paciente(
    ...
    apellidos_param VARCHAR(100),
    telefono_param VARCHAR(20),
    fecha_nacimiento_param DATE,
    ciudad_param VARCHAR(100),
    calle_param VARCHAR(255),
    email_param VARCHAR(100),
    pin_param VARCHAR(50)
) RETURNS void AS $$
BEGIN
    ...
    IF id_direccion_existente IS NULL THEN
        ...
        SET id_direccion_existente = CURRVAL(pg_get_serial_sequence('Direccion', 'Id_direccion'));
    END IF;
...
CREATE OR REPLACE FUNCTION Insertar_Hospital(
    ...
    ciudad_param VARCHAR(50),
    calle_param VARCHAR(50)
) RETURNS void AS $$
BEGIN
    DECLARE id_direccion_existente INT;
...
BEGIN
    ...
    SET v_diagnostico_id = CURRVAL(pg_get_serial_sequence('Direccion', 'Id_direccion'));
...
CREATE OR REPLACE FUNCTION `Crear_Citas`() RETURNS void AS $$
BEGIN
...
DELIMITER //
CREATE OR REPLACE FUNCTION Asignar_Cita(
...
BEGIN
    ...
    WHERE Email = email_in AND P= pin_in;
...
CREATE OR REPLACE FUNCTION Obtener_Departamentos_Hospitales_Cursor(
    id_departamento_param INT
) RETURNS void AS $$
BEGIN
    IF id_departamento_param IS NULL OR id_departamento_param = 0 THEN
        ...
        FROM 
            Departamento d
            JOHospital h ON d.Id_hospital = h.Id_hospital
            JODireccion dir ON h.Id_direccion = dir.Id_direccion;
    ELSE
...
CREATE OR REPLACE FUNCTION Obtener_Hospitales_Cursor(
    p_Id_hospital INT
) RETURNS void AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION Obtener_Medicos_Cursor(
    id_medico_param INT
) RETURNS void AS $$
BEGIN
    IF id_medico_param IS NULL OR id_medico_param = 0 THEN
        ...
        FROM 
            Medico m
            JODireccion dir ON m.Id_direccion = dir.Id_direccion  
            JODepartamento d ON m.Id_departamento = d.Id_departamento
            JOHospital h ON d.Id_hospital = h.Id_hospital;
    ELSE
...
-- -------------------------------
DELIMITER //
CREATE OR REPLACE FUNCTION Obtener_Pacientes_Cursor(id_paciente_param INT) RETURNS void AS $$
BEGIN
    IF id_paciente_param IS NULL THEN
        ...
        FROM 
            Paciente p
        JODireccion dir ON p.Id_direccion = dir.Id_direccion;
    ELSE
...
CREATE OR REPLACE FUNCTION Obtener_Citas_Pendientes_Cursor(
    p_hospital VARCHAR(255),
    p_departamento VARCHAR(255),
    p_fecha DATE  -- Cambiado a tipo DATE
) RETURNS void AS $$
BEGIN
    SELECT 
        c.Id_Cita, 
    ...
    FROM 
        Cita c
        JOMedico m ON c.Id_medico = m.Id_medico
        JODepartamento d ON m.Id_departamento = d.Id_departamento
        JOHospital h ON d.Id_hospital = h.Id_hospital
    WHERE 
        h.Nombre = p_hospital 
...
-- -------------------------------
DELIMITER //
CREATE OR REPLACE FUNCTION Obtener_Citas_Paciente(paciente_id INT) RETURNS void AS $$
BEGIN
    SELECT 
        c.Id_Cita,
        c.Fecha,
...
CREATE OR REPLACE FUNCTION Editar_Medico(
    id_medico_param INT,
    nombre_param VARCHAR(100),
    apellidos_param VARCHAR(100),
    telefono_param VARCHAR(20),
    fecha_nacimiento_param DATE,
    ciudad_param VARCHAR(100),
    calle_param VARCHAR(100),
    email_param VARCHAR(255),
    pin_param VARCHAR(50),
    departamento_param VARCHAR(100),
    hospital_param VARCHAR(100)
) RETURNS void AS $$
BEGIN
    DECLARE id_direccion_existente INT;
...
CREATE OR REPLACE FUNCTION Editar_Departamento(
    p_id_departamento INT,
    p_nombre_hospital VARCHAR(255),
    p_nombre_departamento VARCHAR(255),
...
CREATE OR REPLACE FUNCTION Editar_Hospital(
    p_Id_hospital INT,
...
BEGIN
    ...
    IF v_Id_direccion IS NULL THEN
        ...
        SET v_Id_direccion = CURRVAL(pg_get_serial_sequence('Direccion', 'Id_direccion'));
    END IF;
